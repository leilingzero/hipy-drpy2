...
class Spider(Spider):
    ...
    def proxy_img(self, url):
        if not url or not url.startswith('http') or '127.0.0.1' in url or '/proxy' in url:
            return url
    ...
    def _img(self, url, domain=None):
        ...
        if url.startswith('http'):
            ...
            full = d + url.lstrip('/')
        try:
            r = requests.get(full, timeout=10, verify=False, headers={'User-Agent': self.UA})
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        pc = page + 1 if items else page
        total = pc * 20
        return {'page': page, 'pagecount': pc, 'limit': 20, 'total': total, 'list': items}
    ...
    def searchContent(self, key, quick, pg="1"):
        ...
        if isinstance(data, dict):
            ...
            items = data
        pc = page + 1 if items else page
        return {'list': self._parse_list({'domain': domain, 'data': items}), 'page': page, 'pagecount': pc, 'limit': 20, 'total': len(items)}
